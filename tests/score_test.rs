...
mod tests {
    ...
    #[test]
    fn scores_greater_than_zero_and_shows_match() {
        let range =  match_substring("a", "a");
        assert_eq!(&range[..], "a");
    }

    #[test]
    fn match_range_ends_in_non_inclusive() {
        let range = match_substring("ana.gemspec", "asp");
        assert_eq!(&range[..], "a.gemsp");
    }

    #[test]
    fn no_match_when_the_choice_is_emtpy() {
        let input = "".to_string();
        assert_eq!(do_score(&input, "a"), None);
    }

    #[test]
    fn no_match_if_query_is_longer_than_the_choice() {
        let input = "a".to_string();
        assert_eq!(do_score(&input, "aaaaa"), None);
    }

    #[test]
    fn no_match_if_query_does_not_match_at_all() {
        let input = "a".to_string();
